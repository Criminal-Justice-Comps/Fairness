...
def main():
    mtrx = get_confusion_matrix(test_data,test_guesses)
...
#          dataset where: a=(X=0,C=1), b=(X=1,C=1), c=(X=0,C=0),d=(X=0,C=1)
def get_confusion_matrix(all_data,guesses):
    ...
    for i in range(len(all_data)):
        ...
        if (c == 1) and (x != X_TGT_VALUE):     # a: if (c=1 and x=0)
            cnfsn_matrix[0] += 1
        elif (c == 1) and (x == X_TGT_VALUE):   # b: if (c=1 and x=1)
            cnfsn_matrix[1] += 1
        elif (c == 0) and (x != X_TGT_VALUE):   # c: if (c=0 and x=0)
            cnfsn_matrix[2] += 1
        elif (c == 0) and (x == X_TGT_VALUE):   # d: if (c=0 and x=1)
            cnfsn_matrix[3] += 1
...
def get_sensitivity(confusion_matrix):
    d = confusion_matrix[3] # (X=1) and (C=0)
    b = confusion_matrix[1] # (X=1) and (C=1)
    if (b+d==0):
        return 0
    return d/(b+d)
...
def get_specificity(confusion_matrix):
    a = confusion_matrix[0]
    c = confusion_matrix[2]
    if (a+c==0):
        return 0
    return a/(a+c)
